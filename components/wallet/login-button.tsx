	...
	const renderButton = (currentState: LoginState) => {
		switch (currentState) {
			...
			case 'errorSigningIn':
				return (
					<>
						<Button onClick={handleSignature} className='w-full h-12 bg-white text-black font-medium rounded-xl transition-colors border border-black/10' disabled={false}>
							{t('retrySignature')}
						</Button>
					</>
				);
			...
			case 'signing':
				return (
					<div className='w-full h-12 bg-white text-black font-medium rounded-xl opacity-90 flex items-center justify-center pointer-events-none select-none hover:cursor-not-allowed border border-black/10'>
						<Loader2 className='w-4 h-4 mr-2 animate-spin' />
						{t('signing')}
					</div>
				);
			...
			case 'signed':
				return (
					<div className='w-full h-12 bg-white text-black font-medium border border-black/10 rounded-xl opacity-90 flex items-center justify-center pointer-events-none select-none hover:cursor-not-allowed'>
						✓ {t('loginSuccess')}
					</div>
				);

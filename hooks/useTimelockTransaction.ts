		...
		async (params: TimelockTransactionParams): Promise<TransactionResult> => {
			return executeTransaction(async () => {
				...
				try {
					...
					// Dismiss loading toast and show a new success/in-progress toast
					if (showToasts && toastId !== undefined) {
						createToastNotification.dismiss(toastId);
						createToastNotification.success(t('transactionInProgress'));
					}
					...
					// Dismiss loading toast (if any) and show an appropriate error toast
					if (showToasts && toastId !== undefined) {
						createToastNotification.dismiss(toastId);
						if (message.includes('user rejected') || message.includes('denied')) {
							createToastNotification.error(t('transactionRejectedByUser'));
						} else if (message.includes('insufficient funds')) {
							createToastNotification.error(t('insufficientFundsForTransaction'));
						} else if (message.includes('gas')) {
							createToastNotification.error(t('transactionFailedDueToGasIssues'));
						} else if (message.includes('failed on chain') || message.includes('reverted')) {
							createToastNotification.error(t('transactionRevertedOnChain'));
						} else {
							createToastNotification.error(t('transactionFailed'));
						}
					}

					// Then throw mapped errors for upstream handlers
					if (message.includes('user rejected') || message.includes('denied')) {
					...
					if (message.includes('failed on chain') || message.includes('reverted')) {
						throw new Error(t('transactionRevertedOnChain'));
					}

					throw new Error(message);
				} finally {
					if (showToasts && toastId) {
						// Let the success or error toast remain visible; auto-dismiss is handled by the toast library
					}
				}
			});
		},

    ...
    preprocessParams(paramTypes: ParamType[], params: ParamValue[]): ParamValue[] {
        return params.map((param, index) => {
            ...
            if (typeof param === 'string') {
                const isArrayType = paramType.endsWith('[]');
                if (isArrayType) {
                    ...
                    if (elementType === 'address') {
                        return arrayValue.map(v => this.normalizeAddress(String(v)));
                    }

                    if (elementType.startsWith('uint') || elementType.startsWith('int')) {
                        return arrayValue.map((v: any) => v.toString());
                    }

                    if (elementType === 'bool') {
                        return arrayValue.map((v: any) => {
                ...
                // Handle tuple and tuple[] types using JSON input
                if (paramType === 'tuple' || paramType.startsWith('tuple(') || paramType === 'tuple[]' || paramType.startsWith('tuple[]')) {
                    try {
                        const parsed = JSON.parse(param);

                        // For tuple[] we expect an array of objects/tuples
                        if (paramType.includes('[]')) {
                            if (!Array.isArray(parsed)) {
                                throw new Error('Expected JSON array for tuple[] parameter');
                            }
                            return parsed;
                        }

                        // For single tuple we accept either an object or an array
                        return parsed;
                    } catch (e) {
                        throw new Error(`Invalid JSON for tuple parameter: ${(e as Error).message}`);
                    }
                }
            ...
            if (Array.isArray(param) && paramType.endsWith('[]')) {
                const elementType = paramType.slice(0, -2);
                if (elementType === 'address') {
                    return (param as any[]).map(v => this.normalizeAddress(String(v)));
                }
                if (elementType.startsWith('uint') || elementType.startsWith('int')) {
                    return (param as any[]).map(v => v.toString());
                }
            }
